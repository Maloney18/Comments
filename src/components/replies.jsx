...
import { decReplyScore, deleteReply, incReplyScore, toggleDeleteBar, toggleReplyBar, toggleEditBar, updateCommentAndReply, toggleUpvote } from "../dataBase/Features/generalData"
import { useState } from 'react'

const Reply = (props) => {
    const { data } = useSelector(store => store.generalData)
    const dispatch = useDispatch()
    const { user, createdAt, content, score, replyingTo, id, open, deleteTog, edit, upvoted } = props?.replies
    ...
    const write = (e) => {
        ...
        setUpdateRep(prevRep => ({...prevRep, [name]: value}))
    }
                        ...
                        {data.currentUser.username === user.username && edit ? 
                            <div className="update">
                                <textarea 
                                    ...
                                    rows="10"
                                    onChange={(e) => write(e)}
                                    value={updateRep?.content}
                                    placeholder='Add a comment...'
                                ></textarea>

                                <button 
                                    onClick={() => {
                                        dispatch(updateCommentAndReply(updateRep))
                            ...
                            <div className="word"><span className="reply-to">@{replyingTo}</span> {content}</div>
                        }
                    ...
                    <div className="lower">
                        ...
                        { data.currentUser.username === user.username ?
                            <div className="icon-cont flex">
                                <div className="del-icon flex" onClick={() => dispatch(toggleDeleteBar(id))}>
                        ...
                        :
                            <div onClick={() => dispatch(toggleReplyBar(id))} className="reply">
                ...
                <div className="delete-bar-cont flex">
                    <div className="delete-bar">
                        ...
                        <div className="btn-cont flex">
                            <button className="canc-btn second-btn" onClick={() => dispatch(toggleDeleteBar(id))}>NO, CANCEL</button>
                            <button className="del-btn second-btn" onClick={() => dispatch(deleteReply(id))}>YES, DELETE </button>
                        </div>
                    </div>
                </div>
            }
